package net.frank.yangtes.commons.security.shiro.session;
...
import net.frank.yangtes.commons.utils.JedisUtils;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	public void update(Session session) throws UnknownSessionException {
		...
		try {
			...
			logger.error("Execute update error, Input Param [session.id] -> "+
					session.getId() + ", error description:" + e.getMessage(), e);
		} finally {
	...
	public void delete(Session session) {
		...
		try {
			...
			logger.error("Execute delete error, Input Param [session.id]  -> "+
					session.getId() + ", error description:" + e.getMessage(), e);
		} finally {
			JedisUtils.returnResource(jedis);
		}
	...
	public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession){
		...
		try {
			...
			logger.error("Execute getActiveSessions error, Input Param [includeLeave] -> " +
					includeLeave + "\nInput Param [principal] -> " + principal +"\nInput Param [filterSession] -> " +filterSession+ "\nerror description:" + e.getMessage(), e);
		} finally {
			JedisUtils.returnResource(jedis);
		}
		return sessions;
...
//			}
			...
			logger.error("Execute doReadSession error, Input Param [sessionId] -> " + sessionId + ", error description:" + e.getMessage(), e);
		} finally {
			JedisUtils.returnResource(jedis);
		}
