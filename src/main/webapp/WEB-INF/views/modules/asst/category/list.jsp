...
<layout:default title="资产品类管理">
	<link href="${ctxStatic}/treeTable/themes/vsStyle/treeTable.min.css"
		rel="stylesheet" type="text/css" />
	<script src="${ctxStatic}/treeTable/jquery.treeTable.js"
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/asst/list">资产品类列表</a></li>
		<li><a href="${ctx}/asst/category/form">资产品类添加</a></li>
	</ul>
	<sys:message content="${message}" />
	<table id="treeTable" class="table table-hover">
		<thead>
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			<td><a href="${ctx}/asst/category/form?id={{row.id}}">{{row.name}}</a></td>
			<td>{{row.bnsPre}}</td>
			
			<td>
				<a href="${ctx}/asst/category/form?id={{row.id}}">修改</a>
				<a href="${ctx}/asst/category/delete?id={{row.id}}" onclick="return confirmx('要删除该品类及所有子类项么？', this.href)">删除</a>
				<a href="${ctx}/asst/category/form?parentId={{row.id}}">添加下级品类</a> 
			</td>
		</tr>
	</script>
	<script type="text/template" id="treeTableTplHasChild">
